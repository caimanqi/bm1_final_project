...
# train the model
model <- train(losdays2_log ~ is30dayreadmit + ageyear + cindex + evisit + gender + maritalstatus + insurancetype + bpsystolic + heartrate +  respirationrate + bpdiastolic, data=data_no_outliers, trControl=train_control, method='lm')
# summarize results
print(model)
```
...
SLR_is30dayreadmit <- lm(data_no_outliers$losdays2 ~ data_no_outliers$is30dayreadmit)
...
SLR_cindex <- lm(data_no_outliers$losdays2 ~ data_no_outliers$cindex)
Cindex <- cbind(coef(summary(SLR_cindex))[, c(1:2, 4)], confint(SLR_cindex))[-1, ]


SLR_evisit <- lm(data_no_outliers$losdays2 ~ data_no_outliers$evisit)
Evisit <- cbind(coef(summary(SLR_evisit))[, c(1:2, 4)], confint(SLR_evisit))[-1, ]


SLR_ageyear <- lm(data_no_outliers$losdays2 ~ data_no_outliers$ageyear)
Age <- cbind(coef(summary(SLR_ageyear))[, c(1:2, 4)], confint(SLR_ageyear))[-1, ]


SLR_gender <- lm(data_no_outliers$losdays2 ~ data_no_outliers$gender)
Gender <- cbind(coef(summary(SLR_gender))[, c(1:2, 4)], confint(SLR_gender))[-1, ]


SLR_maritalstatus <- lm(data_no_outliers$losdays2 ~ data_no_outliers$maritalstatus)
Marital_Status <- cbind(coef(summary(SLR_maritalstatus))[, c(1:2, 4)], confint(SLR_maritalstatus))[-1, ]

SLR_insurancetype <- lm(data_no_outliers$losdays2 ~ data_no_outliers$insurancetype)
Insurance_Type <- cbind(coef(summary(SLR_insurancetype))[, c(1:2, 4)], confint(SLR_insurancetype))[-1, ]

SLR_bpsystolic <- lm(data_no_outliers$losdays2 ~ data_no_outliers$bpsystolic)
BP_Systolic_mmHg <- cbind(coef(summary(SLR_bpsystolic))[, c(1:2, 4)], confint(SLR_bpsystolic))[-1, ]

SLR_heartrate <- lm(data_no_outliers$losdays2 ~ data_no_outliers$heartrate)
Heart_Rate_bpm <- cbind(coef(summary(SLR_heartrate))[, c(1:2, 4)], confint(SLR_heartrate))[-1, ]


SLR_respirationrate <- lm(data_no_outliers$losdays2 ~ data_no_outliers$respirationrate)
Respiration_Rate_bpm <- cbind(coef(summary(SLR_respirationrate))[, c(1:2, 4)], confint(SLR_respirationrate))[-1, ]


SLR_bpdiastolic <- lm(data_no_outliers$losdays2 ~ data_no_outliers$bpdiastolic)
BP_Diastolic_mmHg <- cbind(coef(summary(SLR_bpdiastolic))[, c(1:2, 4)], confint(SLR_bpdiastolic))[-1, ]

SLR_res <- rbind(Is_30_Day_Readmit, Cindex, Evisit, Age, Gender, Marital_Status, Insurance_Type, BP_Systolic_mmHg, Heart_Rate_bpm, Respiration_Rate_bpm, BP_Diastolic_mmHg)


#MLR
MLR_ghprojrct <- lm(data_no_outliers$losdays2 ~ data_no_outliers$is30dayreadmit + data_no_outliers$cindex + data_no_outliers$evisit + data_no_outliers$ageyear + data_no_outliers$gender +data_no_outliers$maritalstatus +data_no_outliers$insurancetype + data_no_outliers$bpsystolic  + data_no_outliers$heartrate + data_no_outliers$respirationrate + data_no_outliers$bpdiastolic)
...
cbind(round(SLR_res,digits = 3), round(MLR_res, digits = 3)) %>% DT::datatable(caption = 'Table 2: Summary of continuous variable between SLR vs MLR.', container = sketch)
```
